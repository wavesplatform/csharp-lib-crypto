...
namespace csharp_lib_crypto
{
    public partial class WavesCrypto
    {
        ...
        public string Base16Encode(byte[] input)
        {
            return string.Concat(input.Select(x => ((int)x).ToString("x2")));
        }

        public byte[] Base16Decode(string input)
        {
            if (input.StartsWith("base16:"))
                input = input.Substring(7);
            return Enumerable.Range(0, input.Length)
                     .Where(x => x % 2 == 0)
                     .Select(x => Convert.ToByte(input.Substring(x, 2), 16)).ToArray();
        }
    }
}
